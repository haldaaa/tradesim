...
from app.data import (
    fake_entreprises_db, fake_produits_db, fake_fournisseurs_db, 
...
def reset_game():
    ...
    global fake_entreprises_db, fake_produits_db, fake_fournisseurs_db, prix_par_fournisseur, produits_ayant_subi_inflation
    ...
    # Régénérer avec la config par défaut
    generate_game_data(DEFAULT_CONFIG)
    
    print("✅ Partie remise aux valeurs par défaut !")

def generate_game_data(config: Dict[str, Any]):
    """Génère les données du jeu selon la configuration"""
    global fake_entreprises_db, fake_produits_db, fake_fournisseurs_db, prix_par_fournisseur
...
def generate_fournisseurs(config_fournisseurs: Dict[str, Any]):
    ...
    global fake_fournisseurs_db, prix_par_fournisseur
    ...
    for fid in range(1, nombre_fournisseurs + 1):
        ...
        produits_attribués = random.sample(fake_produits_db, min(nb_produits, len(fake_produits_db)))
...
def get_current_config() -> Dict[str, Any]:
    ...
    return {
        "entreprises": {
            "nombre": len(fake_entreprises_db),
            "budget_min": min([e.budget_initial for e in fake_entreprises_db]),
            "budget_max": max([e.budget_initial for e in fake_entreprises_db]),
            "strategies": list(set([e.strategie for e in fake_entreprises_db])),
            "types_preferes": list(set([t.value for e in fake_entreprises_db for t in e.types_preferes]))
        },
        "produits": {
            "nombre": len(fake_produits_db),
            "prix_min": min([p.prix for p in fake_produits_db]),
            "prix_max": max([p.prix for p in fake_produits_db]),
            "actifs_min": 3,  # Valeur par défaut
            "actifs_max": 8,   # Valeur par défaut
            "types": list(set([p.type.value for p in fake_produits_db]))
        },
        "fournisseurs": {
            "nombre": len(fake_fournisseurs_db),
            "produits_min": min([len(f.stock_produit) for f in fake_fournisseurs_db]),
            "produits_max": max([len(f.stock_produit) for f in fake_fournisseurs_db]),
            "stock_min": min([min(f.stock_produit.values()) for f in fake_fournisseurs_db if f.stock_produit]),
            "stock_max": max([max(f.stock_produit.values()) for f in fake_fournisseurs_db if f.stock_produit])
        },
        "simulation": {
...
def show_game_summary(n_tours: int = None):
    ...
    # 3. Fournisseurs
    print(f"\n🏪 FOURNISSEURS ({len(fake_fournisseurs_db)})")
    print("┌" + "─" * 80 + "┐")
    ...
    for fournisseur in fake_fournisseurs_db:
        ...
        for produit_id, stock in fournisseur.stock_produit.items():
            if stock > 0:  # Seulement les produits en stock
                produit = next(p for p in fake_produits_db if p.id == produit_id)
                prix_cle = (produit_id, fournisseur.id)
                if prix_cle in prix_par_fournisseur:
                    prix_fournisseur = prix_par_fournisseur[prix_cle]
    ...
    # 5. Résumé général
    print(f"\n📊 RÉSUMÉ GÉNÉRAL")
    print(f"• {len(fake_entreprises_db)} entreprises avec budgets {min([e.budget for e in fake_entreprises_db]):.0f}€-{max([e.budget for e in fake_entreprises_db]):.0f}€")
    print(f"• {len(fake_fournisseurs_db)} fournisseurs proposant 3-8 produits chacun")
    print(f"• {len(produits_actifs)} produits actifs sur {len(fake_produits_db)} produits totaux")
