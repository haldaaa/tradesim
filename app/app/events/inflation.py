...
REDUCTION_CHANCE = 0.3  # 30% de chance de réduction progressive
...
def reduire_inflation_progressive(tick: int):
    """
    Réduit progressivement l'inflation sur les produits affectés.
    Retourne une liste de logs (dictionnaires) pour jsonl + log_humain.
    """
    if random.random() > REDUCTION_CHANCE:
        return []  # Pas de réduction ce tick
    ...
    # Trouver les produits qui ont subi une inflation
    produits_inflates = list(produits_ayant_subi_inflation)
    
    if not produits_inflates:
        return []  # Aucun produit à réduire

    # Sélectionner aléatoirement un produit à réduire
    produit_id = random.choice(produits_inflates)
    
    for (pid, fid), prix in prix_par_fournisseur.items():
        if pid == produit_id:
            # Réduction progressive (entre 5% et 15%)
            reduction_pourcentage = random.uniform(0.05, 0.15)
            ancien_prix = prix
            nouveau_prix = round(prix * (1 - reduction_pourcentage), 2)
            prix_par_fournisseur[(pid, fid)] = nouveau_prix
            ...
            pourcentage = round(((ancien_prix - nouveau_prix) / ancien_prix) * 100, 1)
            pourcentages.append(pourcentage)

            reduction_logs.append({
                "produit_id": pid,
                "produit_nom": produit.nom if produit else "Inconnu",
                "produit_type": produit.type.value if produit else "Inconnu",
                "fournisseur_id": fid,
                ...
                "reduction_appliquee": reduction_pourcentage
            })
    ...
    if reduction_logs:
        ...
        produit_cible_nom = "Inconnu"
        pourcentage_produit_cible = 0
        if len(reduction_logs) > 0:
            produit_cible_nom = reduction_logs[0]["produit_nom"]
            pourcentage_produit_cible = reduction_logs[0]["pourcentage_reduction"]
        
        message_humain = (
            f"[RÉDUCTION INFLATION] Produit : {produit_cible_nom} - "
            f"Prix réduit: -{pourcentage_produit_cible}%"
        )
        ...
        log_json = {
            ...
            "timestamp": horodatage,
            "timestamp_humain": horodatage_humain,
            "type": "reduction_inflation",
            "description": f"Réduction progressive de l'inflation sur {produit_cible_nom}",
            "modifications": reduction_logs,
            "statistiques": {
                "nombre_modifications": len(reduction_logs),
                "pourcentage_min": min_pourcentage,
                "pourcentage_max": max_pourcentage,
                "pourcentage_moyen": moy_pourcentage
            },
